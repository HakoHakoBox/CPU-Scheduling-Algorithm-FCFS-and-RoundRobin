...
class process {
public:
	int processName = 0; // Will always be one more than the index in eventQueue
	int arrivalTime = 0;
	int turnAroundTime =0;
	int startTime = 0;
	int completionTime = 0;
	int executionTime = 0;
	int originalExecutionTime = 0;
private:
};
...
void roundRobin(process array[], int number_of_processes, int quantiumTime) {
	...
	while (timeRemaining) { //while processes still have execution time remaining
		for (int i = 0; i < number_of_processes; i++) { //goes through the entire eventQueue
			...
			for (int i = 0; i < number_of_processes; i++) {
				/*
				* If a process is found that still has execution time remaining, then the while loop continues for
			   another iteration.
				* If no processes are found with execution time > 0, the loop will not break and time remaining will be false
...
int main() {
	string line;
	ifstream myfile;
	myfile.open("Input_Processes.txt");
	...
	int index = 0;
	int quantiumTen = 10; //amount of time units
	...
	while (myfile >> process_number >> arrival_time >> cpu_time) { //takes each line in the txt and assigns the numbers to their variables
		...
		eventQueue[index].executionTime = cpu_time; // in roundRobin executionTime is used to see how long the cpu has worked on the process
	...
	roundRobin(eventQueue, number_of_processes, quantiumTen); //each process gets 10 time units with the cpu before the next process is loaded
	reset(eventQueue, executionReset, number_of_processes);
	roundRobin(eventQueue, number_of_processes, quantiumFifty); //each process gets 50 time units with the cpu before the next process is loaded
	reset(eventQueue, executionReset, number_of_processes);
	roundRobin(eventQueue, number_of_processes, quantiumHundo); //each process gets 100 time units with the cpu before the next process is loaded
	reset(eventQueue, executionReset, number_of_processes);
}
